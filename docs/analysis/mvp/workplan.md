...
**Delivers:**
- Counterparty CRUD, multi-line draft Invoice creation and draft-only `UpdateDraftInvoice` (VAT breakdown, direction, counterparty reference), `IssueInvoice`, `VoidInvoice`, `ArchiveInvoice` for payment-free issued/overdue/void invoices, draft-only permanent `DeleteDraftInvoice` for payment-free accidental drafts, invoice filtering/details and a Blob-backed attachment collection (zero or one `invoice`, zero or more `other`), `RecordPayment`, `ReconcilePaymentWithTransaction` (second proof of the event backbone — this service replicating Financial Accounts & Ledger's Transaction), and the `MarkInvoicesOverdue` background job.
...
**E2E test:** create a counterparty and multi-line draft invoice, edit its dates and line collection and confirm VAT/total recalculation, filter and open its details, attach an `other` document and confirm the document icon remains hidden, attach and open an `invoice` document through the icon, upload another automatic `other` document, remove the invoice document and confirm the type choice returns, issue the invoice, record a payment, reconcile that payment against a Phase 2 transaction, watch the invoice flip to paid, and confirm it can no longer be archived. Create a separate accidental draft and confirm it is permanently deleted.
