...
Actuals (Expenses, Revenues) and forecasts (Planning) are one comparison problem, consumed together by the Financial Overview screen and owned by the same team. **Decision (2026-09-06): `Budget` is merged into `Plan`** — the MVP screens only expose a "Plans" screen, and `Plan` now carries an optional `allocated_amount`/`start_date`/`end_date` absorbed from the former Budget entity. **Decision (2026-09-13): full reconciliation, CSV/Excel import, and PDF-OCR import are in MVP scope** — Expense and Revenue can each be matched against a Financial Accounts & Ledger Transaction (a new cross-service reference this service must validate, per the Data Consistency Strategy below). Valid structured CSV/Excel rows are confirmed immediately; invalid rows are skipped and returned to the caller. PDF OCR extraction remains pending user confirmation before the record counts toward reports (see `docs/analysis/contracts.md`). Bookkeeping & Planning exposes an application-owned `IDocumentExtractionAdapter`; its production Infrastructure implementation calls Azure AI Document Intelligence, while tests use a deterministic fake. The adapter returns optional extracted fields and diagnostics, so an extraction with no usable amount still produces an empty reviewable draft rather than silently dropping the upload or treating it as a confirmed record.
