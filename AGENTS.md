...
- `Contapop.Application.AppHost/AppHost.cs` wires up Redis (`AddRedis("cache")`), one Aspire-managed PostgreSQL server with five service-owned databases, the Identity & Tenancy service, the scaffold server project, and the Vite frontend.
...
- `Contapop.Identity.Service` contains the initial Identity & Tenancy EF Core model, migrations, transactional-outbox schema, and the `ProvisionTenant` endpoint. `Contapop.Identity.Service.Tests` applies migrations to Testcontainers PostgreSQL and covers provisioning. Cookie authentication, further user commands, and outbox dispatch are still future tasks.
...
- **PII/PCI minimization**: Payment Cards store a user-entered `label` only (e.g. "Visa ending 1234") — never a real card number. Never add a field that would store real card/account numbers.
