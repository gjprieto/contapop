...
- `src/Contapop.Application.slnx` is an Aspire solution with thirteen projects: `Contapop.Application.AppHost`, `Contapop.Experience.Api`, `Contapop.Experience.Api.Tests`, `Contapop.Identity.Service`, `Contapop.Identity.Service.Tests`, `Contapop.Ledger.Service`, `Contapop.Ledger.Service.Tests`, `Contapop.Billing.Service`, `Contapop.Billing.Service.Tests`, `Contapop.Bookkeeping.Service`, `Contapop.Bookkeeping.Service.Tests`, `Contapop.Reconciliation.Service`, and `frontend/frontend.esproj`. Identity & Tenancy has its initial schema and provisioning/authentication flows. Financial Accounts & Ledger includes bank-account, card, transaction, and Transaction Reconciliation Claim APIs. Billing & Invoicing includes its initial schemas, Project/Transaction replicas, and invoice/payment flows. Bookkeeping & Planning has its initial schemas plus Project/Transaction replica infrastructure. The Reconciliation Process API owns the durable operation/retry store for global reconciliation; its dependent-service calls are added with Billing and Bookkeeping reconciliation features.
- `Contapop.Application.AppHost/AppHost.cs` wires up Redis (`AddRedis("cache")`), one Aspire-managed PostgreSQL server with six service-owned databases, the Identity & Tenancy, Ledger, Billing & Invoicing, and Bookkeeping & Planning services, the Reconciliation Process API, the Experience API, and the Vite frontend.
...
- `Contapop.Bookkeeping.Service` contains the initial Bookkeeping & Planning schemas, durable inbox, and Project/Transaction local read models. `Contapop.Bookkeeping.Service.Tests` validates replica delivery, deduplication, and version-gating against Testcontainers PostgreSQL.
...
The AppHost runs Identity & Tenancy, Ledger, Billing & Invoicing, Bookkeeping & Planning, and Reconciliation Process with Dapr sidecars. Install the Dapr CLI before starting the full graph; the `CommunityToolkit.Aspire.Hosting.Dapr` package is pinned to the compatible `13.5.1-beta.748` release while Aspire 13.5.x reaches a stable Dapr-hosting integration.
