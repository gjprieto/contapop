...
- `src/Contapop.Application.slnx` is an Aspire solution with nine projects: `Contapop.Application.AppHost`, `Contapop.Experience.Api`, `Contapop.Experience.Api.Tests`, `Contapop.Identity.Service`, `Contapop.Identity.Service.Tests`, `Contapop.Ledger.Service`, `Contapop.Ledger.Service.Tests`, `Contapop.Reconciliation.Service`, and `frontend/frontend.esproj`. Identity & Tenancy has its initial schema and provisioning/authentication flows. Financial Accounts & Ledger includes bank-account, card, transaction, and Transaction Reconciliation Claim APIs. The Reconciliation Process API owns the durable operation/retry store for global reconciliation; its dependent-service calls are added with Billing and Bookkeeping reconciliation features.
- `Contapop.Application.AppHost/AppHost.cs` wires up Redis (`AddRedis("cache")`), one Aspire-managed PostgreSQL server with six service-owned databases, the Identity & Tenancy service, the Ledger service, the Reconciliation Process API, the Experience API, and the Vite frontend.
