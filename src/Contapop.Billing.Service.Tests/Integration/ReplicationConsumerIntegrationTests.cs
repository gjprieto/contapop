...
public sealed class ReplicationConsumerIntegrationTests : IAsyncLifetime
{
    ...
    public async Task Transaction_consumer_creates_replica_and_ignores_duplicate_delivery()
    {
        ...
        var message = Event("ledger.transaction-recorded.v1", eventId, transactionId, tenantId, 1, occurredAt,
            new { transaction_id = transactionId, tenant_id = tenantId, bank_account_id = bankAccountId, amount = 1250L, date = "2026-09-09", type = "income", description = "Invoice payment", status = "active", created_at = occurredAt });
    ...
    public async Task Transaction_consumer_ignores_an_out_of_order_older_aggregate_version()
    {
        ...
        await ConsumeTransactionAsync(Event("ledger.transaction-recorded.v1", Guid.NewGuid(), transactionId, tenantId, 2, DateTimeOffset.UtcNow,
            new { transaction_id = transactionId, tenant_id = tenantId, bank_account_id = bankAccountId, amount = 500L, date = "2026-09-09", type = "income", description = "Current", status = "active", created_at = DateTimeOffset.UtcNow }));
        await ConsumeTransactionAsync(Event("ledger.transaction-updated.v1", Guid.NewGuid(), transactionId, tenantId, 1, DateTimeOffset.UtcNow,
            new { transaction_id = transactionId, tenant_id = tenantId, bank_account_id = bankAccountId, amount = 100L, date = "2026-09-08", type = "expense", description = "Old", status = "active", updated_at = DateTimeOffset.UtcNow }));
    ...
    private DbContextOptions<BillingDbContext> CreateOptions() => new DbContextOptionsBuilder<BillingDbContext>()
