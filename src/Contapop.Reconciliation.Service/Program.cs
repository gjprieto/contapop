...
builder.Services.AddHttpClient("bookkeeping-service", client => client.BaseAddress = new Uri(builder.Configuration["BookkeepingService:BaseUrl"] ?? "https+http://bookkeeping-service"));
builder.Services.AddScoped<PaymentReconciliationCoordinator>();
builder.Services.AddScoped<FinancialRecordReconciliationCoordinator>();
...
app.MapFinancialRecordReconciliationEndpoints();
app.Run();
