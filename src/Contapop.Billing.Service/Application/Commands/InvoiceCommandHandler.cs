...
public sealed class InvoiceCommandHandler(BillingDbContext database)
{
    public async Task<InvoiceCommandResult<CreatedInvoiceResponse>> CreateAsync(CreateInvoiceCommand command, CancellationToken cancellationToken)
    {
        ...
        var invoice = Invoice.Create(command.TenantId, command.ProjectId, command.CounterpartyId, command.Direction, command.NetAmountMinor, command.TaxRate, command.Date, command.DueDate, DateTimeOffset.UtcNow);
    ...
    public async Task<InvoiceCommandResult<InvoiceStatusResponse>> IssueAsync(IssueInvoiceCommand command, CancellationToken cancellationToken)
    {
        ...
        database.OutboxMessages.Add(OutboxMessage.Create("billing.invoice-issued.v1", "Invoice", invoice.Id, invoice.Version, invoice.TenantId, now, JsonSerializer.Serialize(new { invoice_id = invoice.Id, project_id = invoice.ProjectId, counterparty_id = invoice.CounterpartyId, direction = invoice.Direction, net_amount_minor = invoice.NetAmountMinor, tax_rate = invoice.TaxRate, tax_amount_minor = invoice.TaxAmountMinor, total_amount_minor = invoice.TotalAmountMinor, date = invoice.Date, due_date = invoice.DueDate, issued_at = now })));
